...
export async function documentIngestion(document: File) {
  try {
	...
	const pdfMetadata: DocumentMetadata = {
	  ...
	  modDate: data.info?.ModDate || undefined,
	  pdfFormat: data.info?.PDFFormatVersion || undefined,
	  totalChunks: 0,
	  tableChunks: 0,
	  totalCharacters: 0,
	};
	...
	for (let pageNum = 0; pageNum < totalPages; pageNum++) {
	  ...
	  if (!pageText || pageMarkerPattern.test(pageText)) {
		console.log(
		  `[ingestion] Skipping page ${pageNum + 1}: "${pageText.slice(0, 50)}"`,
		);
		continue;
	  }
	  console.log(`[ingestion] Page ${pageNum + 1}: ${pageText.length} chars`);
	  pageBoundaries.push({
		page: pageNum + 1,
	...
	if (fullText.trim().length === 0) {
	  console.error("[ingestion] PDF has no extractable text (image-based or corrupted)");
	  return {
		success: false,
		message: `The document "${document.name}" contains no extractable text. It may be a scanned/image-based PDF. Please upload a PDF with selectable text.`,
		recommendationQns: [],
		batches: [],
	...
	console.log(
	  `[ingestion] textSplitter produced ${rawChunks.length} chunks after filtering page markers`,
	);

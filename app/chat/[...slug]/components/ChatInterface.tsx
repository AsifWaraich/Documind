...
function extractReferencesFromToolParts(
  message: UIMessage,
  ...
  for (const part of message.parts) {
	if (part.type === "dynamic-tool") {
	  ...
	  if (
		(dp.toolName === "generatorAgent" || dp.toolName === "webSearchTool") &&
		dp.output
	  ) {
		const outputStr =
		  typeof dp.output === "string" ? dp.output : JSON.stringify(dp.output);
		try {
	...
	if (part.type === "tool-invocation") {
	  ...
	  if (
		(ti.toolName === "generatorAgent" || ti.toolName === "webSearchTool") &&
		ti.result
	  ) {
		const resultStr =
		  typeof ti.result === "string" ? ti.result : JSON.stringify(ti.result);
		try {
...
function parseAiResponse(text: string): ParsedResponse {
  try {
	...
	const tryExtract = (startIdx: "first" | "last") => {
	  const jsonStart =
		startIdx === "last" ? cleaned.lastIndexOf("{") : cleaned.indexOf("{");
...
export function ChatInterface({
		  ...
		  if (last) {
			...
			if (dbParsed.references.length > 0) {
			  ...
			  setParsedResponses((prev) =>
				new Map(prev).set(message.id, parsed),
			  );
			}
		  }
		});
	...
	? (() => {
		try {
		  ...
		  return parsed.resetAt
			? new Date(parsed.resetAt).toLocaleDateString()
			: null;
		} catch {
		  return null;
		}
  ...
  const getDisplayText = (message: UIMessage) => {
	...
	if (parsed?.answer) return preprocessLatex(parsed.answer);
	return "";
  };
  ...
  return (
	<section className="flex h-full min-h-0 flex-col overflow-hidden border-border bg-background lg:border-l">
	  {onClose ? (
		<div className="flex items-center justify-between gap-2 border-b border-border bg-background px-3 py-2 lg:hidden">
			  ...
			  <div className="mt-2 flex items-center justify-end gap-2">
				<div className="flex items-center gap-2">
				  <SpeechToText
					...
					onTranscript={handleDictationUpdate}
					disabled={isLoading}
				  />
				  <Button
					type="submit"

...
async function chunkedImport(annotation: any, items: any[]) {
  for (let i = 0; i < items.length; i += IMPORT_CHUNK) {
	annotation.importAnnotations(items.slice(i, i + IMPORT_CHUNK));
	if (i + IMPORT_CHUNK < items.length) await waitFrame();
  }
}
...
export function useAnnotationSync(
  ...
  const importingRef = useRef(true);
  ...
  const flush = useCallback(async () => {
	...
	try {
	  ...
	  const result = await handleSaveAnnotations(documentId, items);
	  if (!result.success) {
	...
	const setup = async () => {
	  ...
	  if (saved.length > 0) {
		const patched = userName
		  ...
		  : saved;
		await chunkedImport(annotation, patched);
	  }
	  ...
	  if (saved.length > 0) {
		setTimeout(() => {
		  if (!cancelled) importingRef.current = false;
		}, 3000);
	  } else {
		importingRef.current = false;
	  }

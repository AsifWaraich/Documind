...
export default function CommentsPage() {
  ...
  useEffect(() => {
	...
	handleGetUserAnnotations(0, 50).then((result) => {
  ...
  const loadMore = async () => {
	setLoadingMore(true);
	const result = await handleGetUserAnnotations(annotations.length, 50);
	setAnnotations((prev) => [...prev, ...result.items]);
	setTotal(result.total);
	setHasMore(result.hasMore);
	  ...
	  {hasMore && (
		<div className="flex justify-center pt-2 pb-6">
		  <button
			...
			className="text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg border border-border px-6 py-2.5 text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50"
		  >
